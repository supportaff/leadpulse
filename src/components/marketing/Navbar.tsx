    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-md border-b border-white/8">
      <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-bold text-lg tracking-tight">MomCare</span>
        </div>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <Link href="/sign-up" className="text-sm bg-white text-black font-semibold px-4 py-2 rounded-xl hover:bg-gray-100 transition">Start Free</Link>
        </div>
      ...
      {open && (
        <div className="md:hidden border-t border-white/8 bg-black px-5 py-4 space-y-3 text-sm">
          ...
          <Link href="/sign-up" className="block bg-white text-black font-semibold text-center py-2.5 rounded-xl" onClick={() => setOpen(false)}>Start Free</Link>
        </div>
      )}
    </nav>
  );
