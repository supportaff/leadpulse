...
import { LayoutDashboard, CreditCard, Settings, X, LogOut, Trash2, TrendingDown, ShieldCheck, Wallet } from 'lucide-react';
...
const nav = [
  ...
  { href: '/debt-planner', label: 'Debt Planner', icon: TrendingDown },
  { href: '/insurance-planner', label: 'Insurance Planner', icon: ShieldCheck },
...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  const handleDelete = async () => {
    if (inputVal.trim() !== token) { setError('Code does not match. Please try again.'); return; }
    setDeleting(true);
    try {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
          <div className="bg-[#0a0a0a] border border-red-500/20 rounded-2xl p-6 w-full max-w-sm space-y-4 shadow-2xl">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-red-500/10 flex items-center justify-center shrink-0"><Trash2 className="w-4 h-4 text-red-400" /></div>
              <div><p className="text-white font-semibold text-sm">Delete account</p><p className="text-gray-500 text-xs">This is permanent and cannot be undone.</p></div>
            </div>
            ...
            <input type="text" value={inputVal} onChange={e => { setInputVal(e.target.value.toUpperCase()); setError(''); }}
              placeholder="Enter code above" autoFocus
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={closeModal} className="flex-1 py-2.5 rounded-xl border border-white/10 text-gray-400 hover:text-white hover:bg-white/5 text-sm transition">Cancel</button>
      ...
      <aside className={cn(
        ...
        <div className="px-5 mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-600 mt-0.5">AI-Powered Financial Advisor</p>
          </div>
        ...
        <div className="px-4 pt-4 border-t border-white/8 space-y-1">
          ...
          <button onClick={openModal} className="w-full flex items-center gap-2.5 px-2 py-2 rounded-lg text-gray-600 hover:text-red-400 hover:bg-red-500/5 transition text-sm">
            <Trash2 className="w-4 h-4 shrink-0" /><span>Delete account</span>
          </button>
        </div>
      </aside>
    </>
