...
// Dummy wallet balance — replace with real DB value once Supabase is wired
const DUMMY_BALANCE = 3; // 3 leads remaining
...
function BillingContent() {
  ...
  const handleTopup = async (packId: string) => {
    ...
    try {
      ...
      // Submit form to PayU hosted checkout
    ...
    <div className="space-y-8">
      ...
      {statusParam === 'success' && (
        <div className="flex items-center gap-3 bg-green-500/10 border border-green-500/20 rounded-xl p-4 text-green-400 text-sm">
          ...
          <span>
            Payment successful! {creditsParam ? <><strong>{creditsParam} leads</strong> added to your wallet.</> : 'Wallet topped up.'}
          </span>
        </div>
      )}
      {statusParam === 'failed' && (
      ...
      <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-3xl font-bold text-white">{DUMMY_BALANCE} <span className="text-base font-normal text-gray-400">leads</span></p>
            <p className="text-xs text-gray-600 mt-0.5">₹{DUMMY_BALANCE * LEAD_COST_INR} equivalent</p>
          </div>
        </div>
        {DUMMY_BALANCE <= 5 && (
      ...
      <div className="bg-white/[0.02] border border-white/8 rounded-2xl p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm text-gray-400">
          <div className="flex gap-3">
            ...
            <p>Top up your wallet with any amount below</p>
          </div>
          <div className="flex gap-3">
            <span className="text-white font-bold text-lg">2</span>
            <p>Browse leads — contact details are hidden by default</p>
          </div>
          <div className="flex gap-3">
            <span className="text-white font-bold text-lg">3</span>
            <p>Click <strong className="text-white">Unlock Lead</strong> to reveal details — costs ₹{LEAD_COST_INR} per lead</p>
          </div>
        </div>
      </div>
            ...
            <div key={pack.id} className={`relative rounded-2xl border p-5 flex flex-col gap-3 transition ${
              'popular' in pack && pack.popular
                ...
                : 'best' in pack && pack.best
                  ? 'bg-white/[0.04] border-white/30 hover:border-white/50'
